...
namespace Scheduling
{
    class CPU
    {
        ...
        public CPU(Disk disk)
        {
            ...
            Done = false;
            TickCount = 0;
            ActiveAddressSpace = new AddressSpace();
            ActiveConsole = new ProcessConsole( 0 ,  "DefaultConsole");
        ...
        private int ExecuteLine(string sLine)
        {
            ...
            else if (asLine[0] == "yield")
            {
                //your code here
                ProgramCounter++;
                Console.WriteLine($"Process {ActiveProcess} yielded the CPU at line {ProgramCounter}.");
                    
                    // שחרור ה-CPU והפעלת הסדרן
                OperatingSystem.TimeoutReached();
        ...
        private void ReadToken(string[] asLine)
        {
            ...
            Console.WriteLine($"ReadToken called: {sFileName}, Token: {iTokenNumber}, Variable: {sParameterName}"); // Debug
